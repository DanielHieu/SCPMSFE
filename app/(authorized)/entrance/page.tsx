...
import CameraCapture, {
  CameraCaptureHandle,
} from "@/components/ui/CameraCapture";
...
export default function EntrancePage() {
  ...
  const [entranceImageDataUrl, setEntranceImageDataUrl] = useState<
    string | null
  >(null);
  ...
  const walkinAreas = useMemo(
    () =>
      parkingLotFull?.areas &&
      parkingLotFull.areas.filter(
        (area: { rentalType: RentalType }) =>
          area.rentalType === RentalType.Walkin,
      ),
    [parkingLotFull],
  );
  ...
  const contractAreas = useMemo(
    () =>
      ...
      parkingLotFull.areas.filter(
        (area: { rentalType: RentalType }) =>
          area.rentalType === RentalType.Contract,
      ),
    [parkingLotFull],
  );
  ...
  useEffect(() => {
    if (parkingLotFull?.parkingSpaces) {
      ...
      const available = parkingLotFull.parkingSpaces.filter(
        (s) => s.status === ParkingSpaceStatus.Available,
      ).length;
      const occupied = parkingLotFull.parkingSpaces.filter(
        (s) => s.status === ParkingSpaceStatus.Occupied,
      ).length;
  ...
  const getSpaceStyles = (status: string, isSelected: boolean) => {
    const baseStyles =
      "relative p-4 text-center font-medium rounded-lg transition-all duration-300 border";
    ...
    const selectedStyle = isSelected
      ? "ring-2 ring-blue-500 transform scale-105 shadow-md"
      : "";
    ...
    try {
      const response = await fetch(
        process.env.NEXT_PUBLIC_API_LOCAL_URL +
          "/api/contracts/license-plate?licensePlate=" +
          licensePlate,
      );
  ...
  const handleEntrance = async () => {
    ...
    if (capturedImage === null) {
      ...
      addNotification(
        "Không thể chụp ảnh. Vui lòng bật camera và thử lại.",
        "warning",
      );
    ...
    try {
      const response = await fetch(
        process.env.NEXT_PUBLIC_API_LOCAL_URL + "/api/parking-lot/entrance",
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            ...
            rentalType:
              contract != null && contract.status == ContractStatus.Active
                ? RentalType.Contract
                : RentalType.Walkin,
            ...
            {entranceImageDataUrl && (
              <div className="mt-4 border border-gray-300 rounded-lg p-2 bg-gray-50">
                <p className="text-xs font-medium text-gray-600 mb-1">
                  Ảnh vào đã chụp (Gửi đi):
                </p>
                <img
                  src={entranceImageDataUrl}
                  alt="Entrance Capture Preview"
          ...
          <div className="space-y-6 overflow-y-auto pr-2">
            {selectedRentalType === RentalType.Walkin && (
              <div className="space-y-6">
                {walkinAreas &&
                  walkinAreas.map((area: Area) => (
                    <div
                      key={area.areaId}
                      className="border border-gray-200 rounded-lg p-4 bg-gray-50"
                    >
                      <h3 className="text-lg font-semibold mb-3 pb-1 border-b text-gray-800">
                        {area.areaName}
                      </h3>
                      <div className="space-y-4">
                        {parkingLotFull &&
                          parkingLotFull.floors
                            .filter(
                              (floor: { areaId: number }) =>
                                floor.areaId === area.areaId,
                            )
                            .map(
                              (floor: {
                                floorId: number;
                                ...
                                <div key={floor.floorId} className="mb-4">
                                  ...
                                  <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3">
                                    {parkingLotFull.parkingSpaces
                                      .filter(
                                        (space: { floorId: number }) =>
                                          space.floorId === floor.floorId,
                                      )
                                      .map(
                                        (space: {
                                    ...
                                    {parkingLotFull.parkingSpaces
                                      ...
                                      .map(
                                        (space: {
                                          ...
                                          const isContractSpace =
                                            contract &&
                                            contract.parkingSpaceId ===
                                              space.parkingSpaceId;
            ...
            {((selectedRentalType === RentalType.Walkin &&
              (!walkinAreas || walkinAreas.length === 0)) ||
              (selectedRentalType === RentalType.Contract &&
                (!contractAreas || contractAreas.length === 0))) && (
              <div className="py-12 text-center text-gray-500">
                ...
                <p className="mt-1 text-sm text-gray-500">
                  Không tìm thấy khu vực phù hợp cho loại hình này.
                </p>
              </div>
            )}
          </div>
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <p>
                    <span className="font-medium">Vị trí đỗ xe:</span>{" "}
                    {contract.parkingSpaceId || "N/A"}
                  </p>
                </div>
                <div>
                  <p>
