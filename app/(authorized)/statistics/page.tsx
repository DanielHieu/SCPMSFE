...
export default function StatisticsPage() {
  ...
  const fetchLogs = useCallback(
    async (pageIndex = result.pageIndex) => {
      setIsLoading(true);
      setError(null);
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_LOCAL_URL}/api/parking-lot/logs?pageIndex=${pageIndex}&pageSize=${result.pageSize}`,
        );
    ...
    [result.pageSize, result.pageIndex],
  );
                        ...
                        {log.isPaid ? (
                          ...
                          <span
                            className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${
                              log.parkingSpaceStatus === "Occupied"
                                ? "bg-blue-100 text-blue-800"
                                : log.parkingSpaceStatus === "Reserved"
                                  ? "bg-yellow-100 text-yellow-800"
                                  : log.parkingSpaceStatus === "Unavailable"
